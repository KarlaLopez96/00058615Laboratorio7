...
//Matriz Bcondicion_e
void calculateBcondicion_eMatrix(Matrix &B){
    zeroes(B,2,6);
...
Matrix createLocalK(int e,mesh &m){
    float condicion_t,condicion_k,condicion_l,condicion_d,condicion_e,Ae,J,D;
    Matrix matrix_t,matrix_k,matrix_l,matrix_d, matrix_e, matrixAe;
    Matrix K,g_matrix,g_matrix_t,Alpha,Bcondicion_e,Alphat,Bcondicion_et,BPrima,BPrimat;
    ...
    calculateBcondicion_eMatrix(Bcondicion_e);
    Ae = calculateLocalArea(e,m);
    transpose(Alpha,Alphat);
    transpose(Bcondicion_e,Bcondicion_et);
    calculateBPrima(BPrima);
    transpose(BPrima,BPrimat);
    transpose(g_matrix,g_matrix_t);

    condicion_t = m.getParameter(MACRO_T);
    productRealMatrix(condicion_t*J/(24*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,Bcondicion_e,2,2,6),6,2,6),matrix_t);
    condicion_k = m.getParameter(MACRO_K);
    productRealMatrix(condicion_k*Ae/(D*D),productMatrixMatrix(Bcondicion_et,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,Bcondicion_e,2,2,6),2,2,6),6,2,6),matrix_k);
    condicion_d = m.getParameter(MACRO_D);
    productRealMatrix((-1)*(condicion_d*Ae)/(8*D*D),productMatrixMatrix(Bcondicion_et,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixAe);
    condicion_l = m.getParameter(MACRO_L);
    productRealMatrix((J*condicion_l)/(9*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrix_e);

    productRealMatrix(J/(6*D),productMatrixMatrix(BPrimat,productMatrixMatrix(Alphat,g_matrix_t,2,2,6),3,2,6),matrix_d);

    zeroes(K,9);
    ubicarSubMatriz(K,0,5,0,5,sumMatrix(matrix_t,matrix_k,6,6));
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrix_e,matrixAe,6,3));
    ubicarSubMatriz(K,6,8,0,5,matrix_d);
    return K;
}
...
Vector createLocalb(int e,mesh &m){
    ...
    float f_x = m.getParameter(MACRO_PSI_X);
    float f_y = m.getParameter(MACRO_PSI_Y);
    float condicion_e = m.getParameter(MACRO_E);
